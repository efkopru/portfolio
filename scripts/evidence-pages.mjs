...
export function featuredSection({ esc, chips }) {
  return `<section class="shell selected-work" id="selected-work" aria-labelledby="selected-heading"><h2 id="selected-heading">Selected work</h2><p class="section-intro">Three projects across geospatial analysis, data delivery, and public-facing software.</p><div class="featured-grid">${featuredWork.map(item => {
    const project = sources.get(item.id);
    return `<article class="featured-card"><img class="featured-image" src="./${item.image}" width="1200" height="630" alt="" loading="lazy" decoding="async"><div class="featured-body"><p class="project-kind">${esc(item.role)}</p><h3><a href="./${project.id}/index.html">${esc(project.title)}</a></h3><p>${esc(project.summary)}</p><p class="featured-result"><strong>${esc(project.metric)}</strong> ${esc(project.metricLabel)}</p>${chips(project.tools.slice(0, 3))}<a class="text-link" href="./${project.id}/index.html">Read case study<span class="sr-only">: ${esc(project.title)}</span> <span aria-hidden="true">→</span></a></div></article>`;
  }).join('')}</div></section>`;
}
