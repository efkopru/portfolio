...
function home() {
  return layout({ title: 'Geospatial Data Science & GIS Development', description: profile.description, body: `<section class="home-intro evidence-intro"><div class="intro-copy"><h1>Esad Kopru</h1><p class="home-specialty">Geospatial data science, data engineering &amp; software engineering</p><p>10+ years building GIS systems, analytical models, and automated data pipelines that turn location information into decisions.</p></div><div class="intro-links"><ul class="home-keywords" aria-label="Selected skills"><li>Python</li><li>SQL</li><li>Machine learning</li><li>ETL pipelines</li></ul><div class="home-actions"><a class="project-button" href="#selected-work">Highlighted work</a><a class="text-link" href="#projects">All projects</a></div></div></section>${featuredSection({ esc, chips })}<div class="shell library-heading"><h2>All projects</h2><p>Explore the original collections, tools, and historical examples.</p></div><section class="shell project-index" id="projects" aria-label="Breakdown of the Projects">${browseCollections.map(c => `<section><h2>${esc(c.heading)}</h2>${projectList(c.entries, './')}</section>`).join('')}</section>` });
}
