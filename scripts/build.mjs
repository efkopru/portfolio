...
<form class="contact-form" action="https://formsubmit.co/${esc(profile.email)}" method="POST" data-contact-form data-contact-endpoint="https://formsubmit.co/ajax/${esc(profile.email)}" aria-label="Contact Esad Kopru" aria-describedby="contact-privacy">
...
<p class="form-note" id="contact-privacy">Your email, subject, and message are sent through FormSubmit to ${esc(profile.email)}. Submission results appear here without leaving this page. <a href="../privacy/index.html">Privacy</a></p>
...
<dialog class="contact-success" data-contact-success aria-labelledby="contact-success-title" aria-describedby="contact-success-description"><h2 id="contact-success-title">Message sent</h2><p id="contact-success-description">Thank you. Your message has been submitted successfully. Refresh this page to send another message.</p><button class="button" type="button" data-contact-success-close autofocus>Close</button></dialog>
