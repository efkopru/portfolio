...
// New asset URLs prevent stale styles, scripts, and updated case diagrams.
const caseDiagrams = [...new Set(siteProjects.flatMap(project => (project.evidenceSections || []).filter(section => section.diagram).map(section => section.diagram.src)))];
// Hash text with LF endings so Windows CRLF checkouts produce the same versions as the Linux host build.
const assetVersions = Object.fromEntries(await Promise.all(['theme.js', 'styles.css', 'script.js', ...caseDiagrams].map(async file =>
...
function layout({ title, description, body, route = '', current = '', noindex = false, redirect = '', socialProject = '' }) {
  ...
  const navigation = navLink('Home', 'index.html') + navLink('Contact', 'contact/index.html') + navLink('Resume', 'resume/index.html') + browseCollections.map(navGroup).join('') + (additionalProjects.length ? navGroup(additionalNav) : '') + navLink('Doctoral Research', 'doctoral-research/index.html');
  return `<!doctype html>
<html lang="en"><head>
...
<header class="site-header"><div class="shell header-inner"><a class="brand" href="${prefix}index.html" aria-label="Esad Kopru home"><img src="${prefix}assets/efk-logo.avif" width="101" height="48" alt="EFK Portfolio"></a><button class="menu-toggle" type="button" aria-expanded="false" aria-controls="site-nav" hidden>Menu</button><nav id="site-nav" class="site-nav${additionalProjects.length ? '' : ' site-nav--main-only'}" aria-label="Main navigation">${navigation}</nav></div></header><div class="rule" aria-hidden="true"></div>
...
function gallery(project, headingLevel = 2) {
  ...
  return `<section class="project-gallery" aria-labelledby="screenshots-heading"><div class="gallery-heading"><h${headingLevel} id="screenshots-heading">Project gallery</h${headingLevel}><p>${esc(note)} ${project.gallery.length} images. Select an image to view it full size.</p></div>${[...groups].map(([group, images]) => `<section class="gallery-group" id="gallery-${esc(group)}">${groups.size > 1 ? `<h3>${esc(names[group] || 'Related examples')}</h3>` : ''}<div class="screenshot-grid">${images.map(figure).join('')}</div></section>`).join('')}</section>`;
}
...
function buildingFootprintStages(project, preprocessing) {
  if (!preprocessing) throw new Error('Building footprint extraction requires its imagery preprocessing stage.');
  const preprocessingAnchor = `part-${preprocessing.id}`;
  return `<nav class="project-parts-nav" aria-label="Project stages"><a href="#${esc(preprocessingAnchor)}">1. Preprocessing</a><span aria-hidden="true">→</span><a href="#deep-learning-extraction">2. Deep learning</a></nav>
<section class="project-stage" id="${esc(preprocessingAnchor)}" aria-labelledby="preprocessing-heading"><h2 id="preprocessing-heading">1. Imagery preprocessing</h2><p>I built a Python and GDAL pipeline to collect imagery, recover missing tiles, and prepare raster mosaics for feature extraction.</p><ol class="stage-steps"><li><strong>Select imagery.</strong> Identify the tiles covering the study area and choose the capture dates.</li><li><strong>Download and check.</strong> Download the tiles, find missing or blank images, and retry only those tiles.</li><li><strong>Build the mosaic.</strong> Georeference and combine the tiles, clip the raster to the study boundary, and create overviews.</li></ol><p><strong>Output:</strong> Prepared raster imagery for the deep-learning stage.</p><p><strong>Coverage result:</strong> The final archived check accounted for all 120,426 expected tiles, with zero unresolved tiles.</p><h3>Tools</h3>${chips(preprocessing.tools.filter(tool => tool !== 'Deep learning'))}<p class="evidence-note">${esc(preprocessing.boundary)}</p><details class="project-stage-details"><summary>Preprocessing workflow and run records</summary><h3 id="${esc(preprocessing.id)}-heading">${esc(preprocessing.title)}</h3><p>${esc(preprocessing.summary)}</p><p>${esc(preprocessing.contribution)}</p><p><strong>Result:</strong> ${esc(preprocessing.result)}</p><p class="case-context">${esc(preprocessing.context)}</p><p>${esc(preprocessing.problem)}</p>${list(preprocessing.approach)}${chips(preprocessing.tools)}${caseEvidence(preprocessing, { esc, headingLevel: 3, idPrefix: `${preprocessing.id}-evidence`, assetVersions })}</details></section>
<section class="project-stage" id="deep-learning-extraction" aria-labelledby="deep-learning-heading"><h2 id="deep-learning-heading">2. Deep-learning extraction</h2><p>The prepared imagery becomes the input for extracting building footprints.</p><h3>What I did</h3><p>${esc(project.contribution)}</p><ol class="stage-steps"><li><strong>Load the imagery.</strong> Use the prepared raster in ArcGIS Pro.</li><li><strong>Run the model.</strong> Apply ESRI’s pretrained US buildings model with its documented default settings.</li><li><strong>Review the footprints.</strong> Inspect the extracted outlines and check for missed buildings or imagery problems.</li></ol><p><strong>Output:</strong> Building outlines extracted from the imagery.</p><p class="case-result"><strong>Result:</strong> ${esc(project.result)}</p><h3>Tools</h3>${chips(project.tools)}<p class="evidence-note">${esc(project.boundary)}</p>${project.links?.length ? `<div class="project-links">${links(project.links)}</div>` : ''}${gallery(project, 3)}${caseDetails(project, { esc, list })}</section>`;
}
...
pages.set('additional-projects/index.html', layout({ title: 'Additional projects', description: additionalProjects.length ? 'Projects outside the main portfolio categories.' : 'Explore projects in the three main portfolio categories.', route: 'additional-projects', body: `<div class="shell text-page"><h1>Additional projects</h1>${additionalProjects.length ? `<p>Projects outside the main categories.</p>${projectList(additionalProjects.map(p => [p.id, p.title]))}` : `<p>All projects are listed in the main categories.</p>${projectList(collections.map(c => [c.id, c.title]))}`}<p class="back-link"><a href="../index.html#projects">← All project collections</a></p></div>` }));
...
// Hostinger serves dist/ with Apache-style configuration and ignores _headers. The content
// policy stays in _headers only until it has been verified against the live embeds and form.
await writeFile(resolve(output, '.htaccess'), `# Generated by scripts/build.mjs.
