...
function projectPart(part) {
  return `<section class="project-part" id="part-${esc(part.id)}" aria-labelledby="${esc(part.id)}-heading"><h2 id="${esc(part.id)}-heading">${esc(part.title)}</h2><p>This imagery preparation workflow comes first. Building footprint extraction continues from the resulting mosaics.</p><p>${esc(part.summary)}</p><p>${esc(part.contribution)}</p><p><strong>Result:</strong> ${esc(part.result)}</p><h3>Tools</h3>${chips(part.tools)}<p class="evidence-note">${esc(part.boundary)}</p>${caseEvidence(part, { esc, headingLevel: 3, idPrefix: `${part.id}-evidence` })}<details class="project-part-details"><summary>Imagery workflow details</summary><p class="case-context">${esc(part.context)}</p><p>${esc(part.problem)}</p>${list(part.approach)}</details></section>`;
}

function projectPage(project) {
  const parent = project.collection;
  const parentProject = siteProjects.find(candidate => candidate.id === project.parentProjectId);
  const parts = siteProjects.filter(candidate => candidate.parentProjectId === project.id);
  const partLinks = parts.length ? `<nav class="project-parts-nav" aria-label="Earlier project stages"><p>Earlier stage: ${parts.map(part => `<a href="#part-${esc(part.id)}">${esc(part.title)}</a>`).join(', ')}</p></nav>` : '';
  return layout({ title: project.title, description: project.summary, route: project.id, body: `<div class="shell detail"><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="../index.html#projects">Projects</a>${parent ? `<span aria-hidden="true">/</span><a href="../${parent.id}/index.html">${esc(parent.title)}</a>` : ''}${parentProject ? `<span aria-hidden="true">/</span><a href="../${parentProject.id}/index.html#part-${esc(project.id)}">${esc(parentProject.title)}</a>` : ''}</nav><header class="project-heading"><h1>${esc(project.title)}</h1><p>${esc(project.summary)}</p>${project.links?.length ? `<div class="project-links">${links(project.links)}</div>` : ''}</header>${caseSummary(project, { esc, chips })}${partLinks}${caseEvidence(project, { esc })}${embeddedApp(project.embed)}${gallery(project)}${parts.map(projectPart).join('')}${projectEvidence(project, { esc })}${caseDetails(project, { esc, list })}${relatedCases(project, siteProjects, { esc })}<p class="back-link"><a href="../index.html#projects">← Back to projects</a></p></div>` });
}
