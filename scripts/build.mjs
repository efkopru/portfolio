...
<div class="theme-toolbar shell" data-theme-toolbar hidden><label for="theme-select">Theme</label><select id="theme-select" data-theme-select><option value="classic">Classic</option><option value="midnight">Midnight</option><option value="evergreen">Evergreen</option></select></div>
<main id="main" tabindex="-1">${body}</main>
<div class="rule" aria-hidden="true"></div><footer class="site-footer"><div class="shell footer-inner"><div class="footer-links"><a href="${prefix}privacy/index.html">Privacy</a></div><p>All rights reserved © EK ${year}</p></div></footer>
...
function contact() {
  return layout({ title: 'Contact', description: 'Contact Esad Kopru about geospatial data science, data engineering, or software engineering roles and projects.', route: 'contact', current: 'contact', body: `<div class="shell text-page contact-page"><header class="page-heading"><h1>Contact me</h1><p class="detail-lead">For geospatial data science, data engineering, and software engineering roles or project work.</p></header><form class="contact-form" action="https://formsubmit.co/${esc(profile.email)}" method="POST" aria-label="Contact Esad Kopru" aria-describedby="contact-privacy"><input type="hidden" name="_url" value="${origin}/contact/"><input type="hidden" name="_template" value="table"><input type="text" name="_honey" tabindex="-1" autocomplete="off" hidden aria-label="Leave this field empty"><div class="form-field"><label for="contact-email">Your email</label><input id="contact-email" name="email" type="email" autocomplete="email" maxlength="254" required></div><div class="form-field"><label for="contact-subject">Subject</label><input id="contact-subject" name="_subject" type="text" maxlength="160" required></div><div class="form-field"><label for="contact-message">Message</label><textarea id="contact-message" name="message" rows="8" maxlength="5000" required></textarea></div><p class="form-note" id="contact-privacy">Your email, subject, and message are sent through FormSubmit to ${esc(profile.email)}. You may be asked to complete a spam check. <a href="../privacy/index.html">Privacy</a></p><button class="button" type="submit">Send message</button></form><p class="contact-fallback">You can also email <a href="mailto:${esc(profile.email)}">${esc(profile.email)}</a>.</p><div class="contact-block"><h2>Public work</h2><a class="text-link" href="${profile.github}">github.com/efkopru <span aria-hidden="true">↗</span></a><p>Based in ${esc(profile.location)}.</p></div></div>` });
}

const pages = new Map([
  ['index.html', home()], ['about/index.html', about()], ['resume/index.html', resume()], ['contact/index.html', contact()],
  ['privacy/index.html', layout({ title: 'Privacy', description: 'Privacy information for the Esad Kopru portfolio.', route: 'privacy', body: `<div class="shell text-page"><header class="page-heading"><p class="eyebrow">Privacy</p><h1>A simple portfolio.</h1></header><section><h2>Site data</h2><p>This site does not set cookies or include analytics and advertising scripts. If you change the theme, only your theme choice is saved in your browser's local storage so it can be restored on later visits. This preference is not sent to a server. Fonts and decorative images are served with the site.</p><p>The hosting provider may process routine request information, such as IP addresses and browser details, to serve and secure the site.</p></section><section><h2>Contact and external links</h2><p>The contact form sends your email address, subject, and message to FormSubmit for delivery to Esad Kopru. Nothing is submitted until you select Send message. FormSubmit provides spam protection and may ask you to complete a CAPTCHA. Its <a href="https://formsubmit.co/privacy.pdf" rel="noopener noreferrer">privacy policy</a> applies. Do not include confidential or sensitive information. Direct email links open your email application.</p><p>GitHub, City GIS applications, and other linked services have their own privacy policies. These services load only when you follow a link, submit the contact form, or select Load interactive application. External applications may use their own cookies, storage, and network services.</p></section></div>` })],
...
for (const [path, content] of [['sitemap.xml', sitemap], ['robots.txt', `User-agent: *\nAllow: /\nSitemap: ${origin}/sitemap.xml\n`], ['_headers', "/*\n  X-Content-Type-Options: nosniff\n  Referrer-Policy: strict-origin-when-cross-origin\n  X-Frame-Options: DENY\n  Permissions-Policy: camera=(), microphone=(), geolocation=()\n  Content-Security-Policy: default-src 'self'; script-src 'self' 'sha256-"+errorPageScriptHash+"'; style-src 'self'; img-src 'self'; font-src 'self'; connect-src 'none'; frame-src https://maps.cityoflewisville.com https://experience.arcgis.com https://lewisville.maps.arcgis.com https://www.arcgis.com; object-src 'none'; base-uri 'self'; form-action https://formsubmit.co; frame-ancestors 'none'\n"]]) {
