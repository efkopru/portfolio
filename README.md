...
- `script.js`: theme selection, dropdown/mobile navigation, the accessible image viewer, click-to-load applications, and legacy hash navigation.
...
The contact page submits the visitor's reply email, subject, and message directly to FormSubmit for delivery to the email configured in `content/portfolio.mjs`. It uses a normal HTTPS POST, keeps CAPTCHA enabled, includes a honeypot, and uses FormSubmit's confirmation/error pages. It does not store submissions in browser storage or show an unverified delivery-success message. A direct email link remains as a fallback.
